...
4. Build an AnnData (cells x genes; ``X`` = the deposited per-million (CPM)
   normalized matrix from syn64333318 -- length-free, sums to 1e6 per sample;
   ``var`` indexed by Ensembl gene_id with Gene_Symbol + biotype as columns).
...
class SoragniBundle:
    ...
    ``gene_biotype`` as columns). ``X`` is the deposited per-million (CPM)
    normalized matrix (length-free, sums to 1e6 per sample).
    """
...
def load_sarcoma_organoids_2024(
    ...
    *,
    ...
    # The deposited matrix (syn64333318) is per-million normalized: every sample
    # column sums to 1e6 (CV ~1e-7) and gene-length coupling is +0.46, i.e. CPM,
    # not the median-of-ratios the paper used only for its Spearman QC. A
    # per-million export cancels any prior size-factor step exactly, so these
    # values equal CPM of the raw STAR counts regardless. Label it honestly.
    adata.uns["normalization"] = "cpm"
