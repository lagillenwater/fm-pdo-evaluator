...
Tahoe drug names via ``drug_metadata``), projected to five columns. With ``--local-dir`` it
aggregates the treated-vs-DMSO means per (line, drug, gene) IN DuckDB (bounded memory + disk
spill), so no single drug's raw rows -- some are tens of millions -- are ever materialized; only
the compact per-(line, drug, gene) result returns, which flows to the ``(delta, key, baseline)``
contract the scorer reads. (The remote hf:// fallback uses a bounded per-drug pyarrow scan.)
...
def _reshape_local_duckdb(
    paths: list[str], target_names: list[str], name_to_cid: dict[str, str], tmp_dir: Path
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Aggregate the local DE parquet to ``(delta, key, base)`` via DuckDB.

    DuckDB computes the mean ``log2FoldChange`` / ``baseMean`` per (line, drug, gene) IN-ENGINE,
    with a bounded memory budget and disk spill, so no single drug's raw rows (some are tens of
    millions -- what OOMed the per-drug pandas path) are ever materialized. Only the compact
    per-(line, drug, gene) result returns, which ``pseudobulk_de_to_deltas`` reshapes unchanged.
    """
    ...
    print(f"aggregating {len(paths)} DE parquet files in DuckDB (bounded, spills to {tmp_dir}) ...")
    de = con.execute(
        "SELECT gene_name, avg(log2FoldChange) AS log2FoldChange, avg(baseMean) AS baseMean, "
        "Cell_ID_DepMap, drug FROM read_parquet(?) "
        "WHERE drug IN (SELECT unnest(?)) GROUP BY gene_name, Cell_ID_DepMap, drug",
        [paths, target_names],
    ).df()
    print(f"  {len(de):,} aggregated (line, drug, gene) rows")
    return pseudobulk_de_to_deltas(de, name_to_cid)
...
def main() -> None:
    ...
    # Build (delta, key, base). --local-dir aggregates in DuckDB (bounded, no raw-row materialize
    # -- THE ROBUST PATH); the remote hf:// fallback scans per drug in pyarrow (can OOM on a large
    # drug). Either way the DE table is a flat no-drug-partition shard set, so this is one process.
    if args.local_dir:
        ...
        real_delta, real_key, base = _reshape_local_duckdb(
            paths, target_names, name_to_cid, local.parent / "duckdb_tmp"
        )
    else:
        fs = HfFileSystem()
        ...
        # bounded per-drug scan (low readahead, no pre_buffer); prefer --local-dir on a big drug.
        scan_opts = {
            "columns": DE_COLS,
            "batch_size": 64_000,
            "batch_readahead": 1,
        ...
        for i, name in enumerate(target_names, start=1):
            de = (
                dset.scanner(filter=pads.field("drug") == name, **scan_opts)
                .to_table()
