...
PubChem CID: **32 drugs** (`data/static/tahoe_target_cids.txt`, on Alpine, in the repository
checkout; not tracked on this branch — generated by `scripts/alpine/00_target_cids.sbatch`). The
32 CIDs resolve to 33 Tahoe drug names (Trametinib appears additionally as its `DMSO_TF`
solvate), which over the 50 lines gives 1,650 candidate (line, drug) conditions. Ribociclib's
rows carry a single plate everywhere in the pool and cannot be split into replicate halves, so it
contributes zero scored pairs: the scored pool is 31 of the 32 declared compounds plus the
solvate name variant, and rung 0's 1,600 scored pairs is 1,650 minus those 50 unscoreable
Ribociclib rows, not 32 × 50 (the two happen to coincide numerically). The restriction exists so
the delta rungs and the viability rungs (GDSC2, rung 4) score the same compounds.
