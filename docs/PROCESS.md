...
- **Every task ships an executable verification entry point.**
  A script that recomputes each promoted claim from the committed artifacts alone — hashes against the provenance record, statistics from the committed tables, the summary's evidence table checked against the artifacts mechanically — printing claim / recomputed / pass-fail, runnable on a laptop in about a minute, plus a thin notebook wrapper committed **without outputs** so the reviewer executes it and watches the checks pass themselves.
  A continuous integration test runs the same checks, so the branch stays green independent of anyone opening the notebook.
