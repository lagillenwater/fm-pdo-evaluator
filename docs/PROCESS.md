...
- **Committed evidence is sized to the claim it supports, not to what the run produced.**
  A table is committed when a promoted claim is re-derived from it (the per-condition correlations, the null draws, the screen composition); an artifact that is illustrative rather than load-bearing is committed subsampled, seeded so a rerun reproduces it, and carries both its own recomputable statistic and the authoritative full-data one.
